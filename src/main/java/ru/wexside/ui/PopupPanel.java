...
public class PopupPanel extends GuiElement {
      ...
      if (this.openingProgress <= 0.01F) {
         ...
         if (renderer != null) {
            ...
            ClippedLayerRenderer.process(
               renderer, matrix, bounds.getX(), bounds.getY(), bounds.getWidth(), bounds.getHeight(), 16.0F, animated, opacity, localMatrix -> {
                  this.renderSurface(localMatrix, renderer);
                  this.renderPopup(delta, localMatrix, renderer);
               }
            );
         }
   ...
   private void renderSurface(Matrix4f matrix, GuiDrawApi renderer) {
      ...
      renderer.drawRoundedRectangleOutlined(
         matrix, 0.0F, 0.0F, width, height, 8.0F, 0.75F, ColorUtils.withAlpha(ThemeColors.backgroundPrimary(), 0.0F), ThemeColors.borderPrimary()
      );
   }
